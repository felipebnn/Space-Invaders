...
private:
	...
	void updateUniformBuffer() {
		...
		ubo.model = glm::rotate(glm::mat4(), deltaTime * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	void createShaderModule(const std::vector<char>& code, VDeleter<VkShaderModule>& shaderModule) {
		...
		std::vector<uint32_t> codeAlligned(code.size() / sizeof(uint32_t) + 1, 0);
	...
	void createTextureImage() {
		...
		std::unique_ptr<stbi_uc, decltype(&stbi_image_free)>pixels { stbi_load("textures/texture.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha), &stbi_image_free };
	...
	void createDescriptorPool() {
		...
		if (vkCreateDescriptorPool(device, &poolInfo, nullptr, descriptorPool.replace()) != VK_SUCCESS) {
			throw std::runtime_error("failed to create descriptor poll!");
		}
	}
...
// file:///home/nardi/Downloads/Vulkan%20Tutorial.pdf
// The geometry has been colored using per-vertex colors so far, whi
